...
puts "\n Enter the Protocol no. to be used.\n 1.AODV \n 2.DSR \n 3.DSDV \n Enter Value::"
flush stdout
...
puts "\n Enter correct protocol name."
   exit 1
}
puts "------------------------------------------------------------------------"
puts "\n Enter the TCP variant to be used \n
1.TCP/Reno
2.TCP/Newreno
3.TCP/Sack1
4.TCP/Vegas
5.TCP/Fack
6.TCP/Linux"
puts "Enter value for tcp variant: "
flush stdout
...
puts "Enter correct tcp variant name. "
   exit 1
}
...
set filenames $val(type_of_routing_protocol)_$val(tcp_variant_input)
...
# Instruct the simulator object ns to save all simulation data into the trace
...
# Topology contains the value of node position for each node in format of x_pos(node_num) and y_pos(node_num)
source ./topology.tcl
...
proc simulation_stop_time {} {
	...
	# Set the filenames here as AODV_Reno.nam and so on
	set filenames nam_files/$val(type_of_routing_protocol)_$val(tcp_variant_input).nam

    #This command flushes the trace buffer and is typically called before the simulation run ends.
    $ns flush-trace
    close $trace_file
    close $namtrace_file
    set delay_file_to_save delay_files/$val(type_of_routing_protocol)_$val(tcp_variant_input).csv
    set pdr_to_save pdr_files/$val(type_of_routing_protocol)_$val(tcp_variant_input).csv
    set drop_to_save drop_files/$val(type_of_routing_protocol)_$val(tcp_variant_input).csv
   # set instant_thro inst_thro_files/$val(type_of_routing_protocol)_$val(tcp_variant_input).csv
  ...
  #  exec awk -f instant_throu.awk $trace_file_path > $instant_thro &

    puts "\n Simulation done, Trace files created and NAM instance created... Simulation is ending"
	exec nam $filenames &
	exit 0
}
$ns run
